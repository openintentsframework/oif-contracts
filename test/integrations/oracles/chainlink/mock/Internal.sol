...
/// @dev
/// https://github.com/smartcontractkit/chainlink-ccip/blob/632b2acd4f2b203fe4cdd3e93ac0e1303a10ff56/chains/evm/contracts/libraries/Internal.sol
...
library Internal {
    ...
    /// You only need to use this address if the minimum gas limit specified by the user is not actually enough to
    /// execute the
    ...
    /// @dev Any2EVMRampMessage struct has 10 fields, including 3 variable unnested arrays, sender, data and
    /// tokenAmounts.
    ...
    /// @notice This methods provides validation for TON User-friendly addresses by ensuring the address is 36 bytes
    /// long.
    ...
    struct Any2EVMTokenTransfer {
        // The source pool EVM address encoded to bytes. This value is trusted as it is obtained through the onRamp. It
        // can
    ...
    /// Note: hash(Any2EVMRampMessage) != hash(EVM2AnyRampMessage), hash(Any2EVMRampMessage) != messageId due to
    /// encoding
    ...
    /// @dev Holds a merkle root and interval for a source chain so that an array of these can be passed in the
    /// CommitReport.
    /// @dev RMN depends on this struct, if changing, please notify the RMN maintainers.
    /// @dev inefficient struct packing intentionally chosen to maintain order of specificity. Not a storage struct so
    /// impact is minimal.
