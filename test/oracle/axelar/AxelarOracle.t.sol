...
contract AxelarGatewayMock {
    ...
    function validateContractCall(
        ...
        if (commandId == keccak256(bytes("commandId"))) {
            return true;
        }
...
contract AxelarOracleTest is Test {
    ...
    function getHashesOfEncodedPayloads(
        ...
        external
        pure
        returns (bytes32 application, bytes32[] memory payloadHashes)
    {
        (application, payloadHashes) = MessageEncodingLib
            .getHashesOfEncodedPayloads(encodedMessage);
    }
    ...
    function test_submit_NotAllPayloadsValid(
        ...
        (output, payloads[0]) = _getMandatePayload(
            sender,
            amount,
            recipient,
            orderId,
    ...
    function test_handle_works(
        ...
        bytes memory message = this.encodeMessageCalldata(
            address(_outputSettler).toIdentifier(),
            payloads
        );

        (bytes32 application, bytes32[] memory payloadHashes) = this
            .getHashesOfEncodedPayloads(message);
